...
internal nginx のアクセスログは、調査に要るリクエストだけを残す。対象は 5xx、client の切断 (499)、応答に 1 秒以上かかったもので、判定した種別 (`bot=`)、制限の結果 (`limit=`)、応答時間 (`rt=` / `urt=`) を付ける。全リクエストの記録は外側の nginx が持っているので、ここで全件を残しても重複になるうえ、crawler の flood 時には数時間でローテーションに押し出される。429 で弾いたリクエストも 1 件ずつは残さない。
